...
/// <summary>
/// Code-behind for the ReviewSessionView.
/// Handles dynamic formatted text parsing (*sc ... *ec) and local media file rendering.
/// </summary>
public partial class ReviewSessionView : ReactiveUserControl<ReviewSessionViewModel>
{
    public ReviewSessionView()
    {
        InitializeComponent();
        ...
        {
            // Bind header texts
            this.OneWayBind(ViewModel, vm => vm.State.CategoryName, v => v.CategoryNameText.Text);
            this.OneWayBind(ViewModel, vm => vm.State.TopicName, v => v.TopicNameText.Text);
            
            // TopicName visibility
            this.WhenAnyValue(x => x.ViewModel!.State.TopicName)
            ...
            this.WhenAnyValue(x => x.ViewModel!.State)
                .Subscribe(Observer.Create<ReviewSessionState>(UpdateUI))
                .DisposeWith(disposables);
        });
    }

    private void UpdateUI(ReviewSessionState state)
    {
